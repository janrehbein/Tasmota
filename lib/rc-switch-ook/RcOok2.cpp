 ...
 */
    ...
    void DecodeOOK::transfer(byte buffer[], byte &pos) {
        // byte pos;
    ...
    void DecodeOOK::sprint(const char * s, char * d) {
        ...
        for (int i = 0; i < pos; i++)
        {
          Serial.print(v[data[i] >> 4]);
          Serial.print(v[data[i] & 15]);
        }
        Serial.print("\n");
 ...
 */
	...
	KW9019Decoder::KW9019Decoder () {}

	int KW9019Decoder::decode (word width) {

        if (width > KW9010_SYNC - 2 * KW9010_GLITCH && width < KW9010_SYNC + 2 * KW9010_GLITCH) {

            if (total_bits >= KW9010_MESSAGELEN) {
                // all bits received
                return 1;
            }
        ...
        } else if (width > KW9010_ONE - KW9010_GLITCH && width < KW9010_ONE + KW9010_GLITCH) {
            gotBit(1);

        } else if (width > KW9010_ZERO - KW9010_GLITCH && width < KW9010_ZERO + KW9010_GLITCH) {
            gotBit(0);

        } else {
            // received bit is not a SYNC, ONE or ZERO bit
            // do nothing, reset is not working here
